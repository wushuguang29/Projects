	...
	<view class="input-box-all" :style="{ marginTop: marginTop, marginBottom: marginBottom}" :class="set">
		<view class="input-box align-items flex">
			...
			<view class="input-box-center" v-if="type==='password'">
				<input @input="keyInput" :placeholder="placeholder" :disabled="disabled" :value="v" type="text" :maxlength="maxLength" class="input-box-center-text" v-show="isShowEye"/>
				<input @input="keyInput" :placeholder="placeholder" :disabled="disabled" :value="v" type="password" :password="true" :maxlength="maxLength" class="input-box-center-text" v-show="!isShowEye"/>
			</view>
			<!-- 非密码输入框类 -->
			<view class="input-box-center" v-else>
				<input @input="keyInput" :placeholder="placeholder" :disabled="disabled" :value="v" :type="type" :maxlength="maxLength" class="input-box-center-text" />
			</view>
			<!-- 清空输入框 -->
			<view class="input-box-clear" v-show="clearShow && v" @click="clear">
				×
			</view>
			<!-- 密码显示隐藏开关 -->
				...
				if (!flag) {
					...
					if (this.verification && this.verification.length > 0) {
						for (let i = 0; i < this.verification.length; i++) {
							...
							// 判断校验是否正确
							const verificationResult = inputVerification.verificationFuc({name: verificationName, value: this.inputValue})
