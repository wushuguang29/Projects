<template>
	<view class="content">
		...
		<view class="text-area" :style='{"background-image":"url("+img+")"}'>
			...
			<view class="zixun" @tap='zx'><span class="zixun_text">咨询</span></view>
		...
		<view class="tan-xf" v-if="!flag">
			...
			<view class="tan-rigth" @click="goto('/pages/index/login')">
				<view style="padding: 0.5rem 0 0 0;padding-bottom: 0.5rem;padding-bottom: 0.6rem;">
					<h3 style="font-size: 43rpx;font-weight: 500;padding: 0 0 0 5%;">登录/注册</h3>
				</view>
				<view><span style="font-size:27rpx;padding:0.5rem 0 0 9px;wxcs_style_padding:0.5rem 0 0 23rpx;padding-bottom:0.6rem;color: #adadad;">请登录以获得更好的测评服务</span></view>
			</view>
		</view>
		<view class="tan-xf" v-if="flag">
			<view class="tan-left" @click="avater">
				<u-avatar :src="myData.avatar" size="140"></u-avatar>
				<!-- 	<image :src="myData.avatar" style="width:100%;height:100%;border-radius: 50%;"></image> -->
			</view>
	...
	export default {
		...
		methods: {
			goToUrl(val) {
				console.log(val)
				uni.redirectTo({
					url: '/pages/views/physiological/subscription?type=' + val,
				});
			},
			zx() {
				uni.redirectTo({
					url: '/pages/views/advisory/index',
				});
			},
			avater() {
				console.log("1111");
