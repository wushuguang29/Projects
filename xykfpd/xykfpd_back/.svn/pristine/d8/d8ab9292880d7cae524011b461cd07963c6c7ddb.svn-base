...
class QuestionsModel extends RewriteModel
{
    ...
    static function add($param)
    {
        ...
        try {
            ...
            //验证标识码是否存在
            //查询方面是否存在
            $code = Db::table('questions')->where('side',$param['side'])->value('identification_code');
            if(empty($code)){//方面不存在
                //查看标识码是否存在
                $codeExist = Db::table('questions')->where('identification_code',$param['identification_code'])->count();
                if($codeExist > 0){//存在抛出提示
                    return msgReturn(50001,[],'标识码已存在，请勿重复使用');
                }
            }else{//方面以存在
                //判断存在的标识码与现在录得是否一致
                if($code != $param['identification_code']){
                    return msgReturn(50001,[],'标识码不一致，当前“'.$param['side'].'”方面的标识码为'.$code.'输入的为'.$param['identification_code']);
                }
            }
