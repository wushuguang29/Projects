...
class StatisticalAnalysis extends Controller
{
    ...
    public function getList()
    {
        ...
        $sideArr = [];
        if(!empty($search)){
            $dataWhere[] = ['aum.name', 'LIKE', '%'. $search. '%'];
        }
        if(!empty($side)){
    ...
    public function getData($where)
    {
        $evaluating = db('evaluating')->alias('e')
            ...
            ->join('app_users au', 'au.id = aum.app_user_id')
            ->where($where)
            ->where('e.evaluating_status', 1)// 已评测
            ->field([
                'e.id',
    ...
    public function buildShowData($comparisonStandard, $data)
    {
        ...
        foreach ($data as $k => $v) {
            $detail = [];
            $detail['number']     = $i++; //序号
            $detail['user_name']  = $v['user_name']; //用户名
            $detail['user_phone'] = $v['user_phone']; //手机号
            $detail['name'] = $v['name']; //测评人
            $detail['age']  = $v['age']; //年龄
            $detail['sex']  = !empty($v['sex']) ? $config['SEX'][$v['sex']] : ''; //性别
            $detail['degree_education']     = !empty($v['degree_education']) ? $config['DEGREE_EDUCATION'][$v['degree_education']] : ''; //文化程度
            $detail['marital_condition']    = !empty($v['marital_condition']) ? $config['MARITAL_CONDITION'][$v['marital_condition']] : ''; //婚姻状况
            $detail['children_condition']   = !empty($v['children_condition']) ? $config['CHILDREN_EDUCATION'][$v['children_condition']] : ''; //子女状况
            $detail['affiliated_disease']   = !empty($v['affiliated_disease']) ? $config['AFFILIATED_DISEASE'][$v['affiliated_disease']] : ''; //所属疾病
            $detail['critical_disease']     = $v['critical_disease']; //所患疾病
            $detail['physical_condition']   = !empty($v['physical_condition']) ? $config['PHYSICAL_CONDITION'][$v['physical_condition']] : ''; //身体状况
            $detail['living_condition']     = !empty($v['living_condition']) ? $config['LIVING_CONDITION'][$v['living_condition']] : ''; //居住环境
            $detail['health_insurance_type'] = !empty($v['health_insurance_type']) ? $config['HEALTH_INSURANCE_TYPE'][$v['health_insurance_type']] : ''; //医保类型
            $detail['source_finance']       = !empty($v['source_finance']) ? $config['SOURCE_FINANCE'][$v['source_finance']] : ''; //经济来源
            $detail['phone'] = $v['phone']; //测评人手机号
    ...
    {
        $res = Db::table('questions')->field('side')->group('side')->select();
