...
class Login extends Controller
{
    ...
    public function alipayAuth(Request $request){
        // echo "<a href='hbuilder://".json_encode($request->param())."'> <h1>跳转</h1> </a>";
        echo "<html>
        <head>
        <meta charset='utf-8' />
        <meta name='viewport' content='width = device-width, initial-scale = 1.0, maximum-scale = 1.0, user-scalable = 0'>
        <style>
        .alipay-box{text-align:center;}
        .alipay-btn{width: 100%;padding: 10px 0;color: #fff;background-color: #4b53e4;font-size: 14px;border: none;}
        </style>
        </head>
        <body>	
        <div class='alipay-box'>
        <p>当前是授权支付宝登录康复App</p>".
        // "<a href='hbuilder://".json_encode($request->param())."'>". //测试
        "<a href='net.lhsoft.lnzhky://".json_encode($request->param())."'>". //安卓打包
        "<button class='alipay-btn'>确认支付宝登录授权</button>
        </a>".
        "<a href='https://xykfpdafter.demo.lhsoft.net/ulink'>". //IOS 
        "<button class='alipay-btn'>测试支付宝登录授权</button>
        </a>
        </body>
        </html>";
        // return "success";
    ...
    public function alipay($data)
    {
        ...
        $aop->format='json';
        $aop->method='alipay.userinfo.share';
        ...
        if(isset($result->alipay_system_oauth_token_response->access_token))
        {
            ...
            if($results->alipay_user_info_share_response->code == 10000)
            {
                ...
                $userInfo['nick_name'] = isset($results->alipay_user_info_share_response->nick_name) ? $results->alipay_user_info_share_response->nick_name : \app\common\service\GenearteName::generate_name()['xm'];
