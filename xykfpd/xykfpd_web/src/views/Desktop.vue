...
export default {
  ...
  data() {
    return {
      ...
      summaryData: [
        {
          evaluating_number: 33,
          evaluating_population_number: 4,
          month: "2021-02",
        },
      ],
      userNumber: 0,
      ...
      polar: {
        ...
        legend: {
          formatter: function (name) {
            if (name == "evaluating_number") {
              return "评估次数";
            }
            if (name == "evaluating_population_number") {
              return "评估人数";
            }
          },
        },
  ...
  created() {
    summary().then((res) => {
      ...
      this.polar.dataset.source = res.data.monthSummary;
  ...
  mounted() {
    ...
    const container = document.getElementsByClassName("desktop-main-box");
    if (container && container[0]) {
      container[0].style.height = windowHeight + "px";
    }
  },
  computed: {
    ...mapGetters(["topMenu"]),
      ...
      meunArr.forEach((each, index) => {
        if (each.front_router_name === routeName) {
          ...
          this.$store.dispatch("createTagPanel", {
            name: each.children[0].front_router_name,
            path: each.children[0].front_router_name,
            title: each.children[0].title,
          });
        }
      });
