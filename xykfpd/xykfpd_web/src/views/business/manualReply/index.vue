          ...
          <el-tab-pane label="基本信息" name="Information">
            <el-tabs v-model="Name" @tab-click="nameClick" style="height: 100%">
              <el-tab-pane
                ...
                :name="'' + index"
                :key="item.id"
                v-for="(item, index) in userMemberArr"
              >
                <el-form
                  :model="formData"
                  label-width="100px"
...
export default {
  ...
  methods: {
    ...
    nameClick(tab, event) {
      ...
      this.formData = this.convertFormData(this.userMemberArr[index]);
    },
    //切换基本信息和对话
    handleClick(tab, event) {
      console.log(tab, event);
      if (tab.name == "Information") {
        getUserMemberList({ id: this.user_id }).then((res) => {
          this.userMemberArr = [...res.data];
          if (this.userMemberArr && this.userMemberArr.length) {
            this.formData = this.convertFormData(this.userMemberArr[0]);
          }
        });
      }
    },
  ...
  created() {
    ...
    try {
      ...
      ws = new WebSocket("wss://kfpd.chengtouyun.com");
