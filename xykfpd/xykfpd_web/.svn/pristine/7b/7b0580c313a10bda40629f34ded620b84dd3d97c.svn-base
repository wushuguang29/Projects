...
export default {
  ...
  methods: {
    ...
    handleDelete: function (options) {
      const { row, index } = options;
      const tableRef = this.$refs.questionTable;
      const pagingRef = tableRef.$refs.needPagingRef;
      ...
      this.deleteGrid({
        ...
        this.getGridData({
          ...
          params: {
            limit: pagingRef.pageSize,
            start:
              pagingRef.pageSize * pagingRef.currentPage - pagingRef.pageSize,
            page: pagingRef.currentPage,
          },
        });
      });
    },
    ...
    //编辑或新增成功后刷新列表
    updateList() {
      this.$refs.searchForm.onResetClick("searchFrom");
      this.getGridData({
        list: QuestionsGetList,
      }); //接口
    },
